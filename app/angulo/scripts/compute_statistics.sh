...
if [ "${ground_truth}" != "" ]; then 
    ...
    # compute the statistics
    disp_statistics out_disp_for_statistics.tif ground_truth_for_statistics.tif 1 > statistics.txt 
    disp_statistics disp_std_bm_for_statistics.tif ground_truth_for_statistics.tif 1 >> statistics.txt 
fi 
