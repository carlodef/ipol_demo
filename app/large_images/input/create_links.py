    ...
    try:
        ...
        if exc.errno == errno.EEXIST and os.path.islink(dst):
            if os.path.realpath(dst) == os.path.realpath(src):
                ...
                print '%s is a link already pointing to %s' % (dst, os.path.realpath(dst))
        else:
            raise
...
def list_images_in_dataset(f):
    """
    Lists the TIF files of a dataset and associates corresponding P and MS.
    ...
    if not ms_files:
        ...
        for ms in ms_files:
            ...
            if matching_pan is None:
                print "no matching panchro image found for %s" % ms
            else:
...
def copy_file_matching_pathname(pattern, src, dest):
    ...
    if not l:
        print 'No filename matching %s in directory %s' % (pattern, src)
    elif len(l) > 1:
        print 'More than one file %s in directory %s: ' % (pattern, src), l
    else:
        shutil.copy(l[0], dest)
        return os.path.abspath(os.path.join(dest, os.path.basename(l[0])))


def create_links(list_of_paths, dest_dir):
    """
    Creates symlinks to a list of pleiades image files and metadata files.

    Args:
        list_of_paths: list of paths to panchro TIF files, produced by the
            list_images_in_dataset function
        dest_dir: directory where to create the symlinks
    """
    for i, f in enumerate(list_of_paths):
        ...
        if not os.path.isfile(dzi):
            print 'file %s not found' % dzi
        elif not os.path.isdir(files):
            print 'directory %s not found' % files
        else:
...
def main(src_dir, dst_dir='pleiades'):
    """
        ...
        if os.path.isdir(dataset_abspath):
            if 'dataset_1' in os.listdir(dataset_abspath):  # the dataset has subdatasets (multidate)
                for subdataset in os.listdir(dataset_abspath):
                    ...
                    create_links(l, os.path.join(dst_dir, dataset, subdataset))
                ...
                create_links(l, os.path.join(dst_dir, dataset))


if __name__ == '__main__':
    if len(sys.argv) < 2:
        print 'usage: %s src_dir [dst_dir (default pleiades)]' % sys.argv[0]
    elif len(sys.argv) == 2:
        main(sys.argv[1])
    else:
